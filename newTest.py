...
def create_issue():
    ...
    if(issue.fields.customfield_11705):
        ...
        #mycursor.execute("Select * from templates where job_title = '%s'" % (titleText.get()))
        ...
        for x in range(len(tempResult)):
            if(tempResult[x] == 1):
                ...
                if(columnName[x] == "A/D (DAI)"):
                    ...
                    tempString = tempString.lower()
                    #childIssue.update(fields={'customfield_11003': tempString})
                elif(columnName[x] == "email" or columnName[x] == "Slack"):
                ...
                #time.sleep(1)
        create_confluence(confluence, employeeText.get(),employeeName, titleText.get())
